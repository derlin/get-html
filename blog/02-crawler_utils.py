...
if os.getenv('RENDER_JS', '0').lower().strip() in ['0', 'false', 'no', 'n', 'off']:
    ...
    _session.browser # trigger the browser creation, and thus the "self.loop = asyncio.get_event_loop()"
    # implement locking to avoid multiple threads trying to use the same event loop.
